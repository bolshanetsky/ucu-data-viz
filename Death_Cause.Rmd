...
ggsave(filename = 'barPlot.png', plot = last_plot(), device = NULL, path = NULL,
...
ggsave(filename = 'barPlot2.png', plot = last_plot(), device = NULL, path = NULL,
  scale = 1, width = NA, height = NA, units = c("in", "cm", "mm"),
  dpi = 300, limitsize = TRUE)
...
df_adj = df_agg3 %>%
  group_by(age_group) %>%
  summarise(death_sum = sum(value))

# df_agg3 = df_agg3 %>%
#  mutate (percent = NA) #%>%
#  mutate(age_total = df_adj[df_adj$age_group == "0-4", "death_sum"])
...
ggsave(filename = 'barPlot3.png', plot = last_plot(), device = NULL, path = NULL,
...
test = apply(df_agg3,1, function(x) x[4] = (df_adj[df_adj$age_group == x[1], "death_sum"]))
  #mutate(age_total = df_adj[df_adj$age_group == age_group, "death_sum"])
```
